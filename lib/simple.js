...
END:VCARD
    ...
    * @returns 
    */
    // conn.cMod = (jid, message, text = '', sender = this.user.jid, options = {}) => {
     ...
     * @param {Object} options
     */
    conn.copyNForward = async (jid, message, forwadingScore = true, options = {}) => {
        let mtype = Object.keys(message.message)[0]
        if (typeof forwadingScore == 'number') {
            let contextInfo = message.message[mtype].contextInfo
            if (!contextInfo) message.message[mtype].contextInfo = {}
            message.message[mtype].contextInfo.forwadingScore = forwadingScore
        }
        return await conn.sendMessage(jid, { ...options, forward: message, force: !!forwadingScore }, { ...options })
    }
