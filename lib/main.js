let { WAConnection: _WAConnection, WA_MESSAGE_STUB_TYPES, MessageType } = require('@adiwajshing/baileys')
...
//Don't remove this number
global.isError = ['6281390658325']
...
if (!opts['test']) setInterval(() => {
  ...
  if (JSON.stringify(global.DATABASE.data) == lastJSON) {
      conn.logger.info('Database is up to date')
  } else {
...
conn.handler = async function (m) {
  try {
    ...
    let isError = global.isError.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
    ...
    let isBotAdmin = bot.isAdmin || bot.isSuperAdmin || false // Are you Admin?
      ...
      if (typeof plugin.before == 'function') if (await plugin.before.call(this, m, {
        ...
        if (m.chat in global.DATABASE._data.chats || m.sender in global.DATABASE._data.users) {
          ...
          if (name != 'unbanuser.js' && user && user.Banneduser) return // Except this
        ...
        m.isCommand = true
        let xp = 'exp' in plugin ? parseInt(plugin.exp) : 5 // XP Earning per command
        if (xp > 99999999) m.reply('Ngecit -_-') // Hehehe
        else m.exp += xp
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    _group : '*Kuy gabung official Group Metro Bot*\n\nhttps://chat.whatsapp.com/Lb4Emjih98rBiCZiZoS2e\n\n*#RoadTo250Member*\n*#izinMin*',
...
if (opts['test']) {
  ...
  rl.on('line', line => conn.sendMessage('6281390658@s.whatsapp.net', line.trim(), 'conversation'))
} else {
  rl.on('line', line => {
    global.DATABASE.save()
...
let filenyakocak = require.resolve(__filename)
fs.watchFile(filenyakocak, () => { 
   fs.unwatchFile(filenyakocak) 
   console.log(chalk.redBright("Update 'main.js'")) 
   delete require.cache[filenyakocak] 
   require(filenyakocak)
})