...
function makeInMemoryStore() {
    ...
    function fromJSON(json) {
        ...
        for (const jid in json.messages)
            messages[jid] = json.messages[jid].map(m => proto.WebMessageInfo.fromObject(m)).filter(m => m?.messageStubType != WAMessageStubType.CIPHERTEXT)
...
const useMultiFileAuthState = baileysMultiFileAuthState ||
    ...
    async function useMultiFileAuthState(folder) {
        ...
        const readData = async (file) => {
            try {
                const data = await promises.readFile(join(folder, file), { encoding: 'utf-8' })
