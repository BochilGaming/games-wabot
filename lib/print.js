import { WAMessageStubType } from '@adiwajshing/baileys-md'
let urlRegex = require('url-regex-safe')({ strict: false })
import PhoneNumber from 'awesome-phonenumber'
let terminalImage = global.opts['img'] ? require('terminal-image') : ''
import chalk, { redBright, black, bgYellow, bgGreen, magenta, green, yellow, blueBright, red, gray } from 'chalk'
...
export default async function (m, conn = { user: {} }) {
  ...
  console.log(`
${redBright('%s')} ${black(bgYellow('%s'))} ${black(bgGreen('%s'))} ${magenta('%s [%s %sB]')}
${green('%s')} ${yellow('%s%s')} ${blueBright('to')} ${green('%s')} ${black(bgYellow('%s'))}
`.trim(),
    me + ' ~' + conn.user.name,
  ...
  if (typeof m.text === 'string' && m.text) {
    ...
    if (log.length < 4096)
      log = log.replace(urlRegex, (url, i, text) => {
        ...
        return i === 0 || end === text.length || (/^\s$/.test(text[end]) && /^\s$/.test(text[i - 1])) ? blueBright(url) : url
      })
    log = log.replace(mdRegex, mdFormat(4))
    if (m.mentionedJid) for (let user of m.mentionedJid) log = log.replace('@' + user.split`@`[0], blueBright('@' + await conn.getName(user)))
    console.log(m.error != null ? red(log) : m.isCommand ? yellow(log) : log)
  }
  if (m.messageStubParameters) console.log(m.messageStubParameters.map(jid => {
    jid = conn.decodeJid(jid)
    let name = conn.getName(jid)
    return gray(PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international') + (name ? ' ~' + name : ''))
  }).join(', '))
...
watchFile(file, () => {
  unwatchFile(file)
  console.log(redBright("Update 'lib/print.js'"))
